...
def hacer_graficas():
    ...
    def metodo_euler(x0, v0, f, dt, tf):
        ...
        #print(f'tiempo del metodo euler:{xTiempo}')
        ...
        print(f'xValue del metodo euler:{xValue}')
    ...
    def ValorMedioPosicion(x0, v0, c, k, m, tf, dt):
        xTiempo = np.arange(0, tf + dt, dt)
        xEsperado = np.zeros(len(xTiempo))
        valor1a = np.zeros(len(xTiempo))
        valor2a = np.zeros(len(xTiempo))
        valor3a = np.zeros(len(xTiempo))
        valor4a = np.zeros(len(xTiempo))
        mcua = m ** 2
        por4 = mcua * 4
        divi = 1/por4
        for i in range(0, len(xTiempo)):
            valor1 = (x0*np.exp((-1/(2*m))*(xTiempo[i]*c)))
            valor2 = (np.cosh(xTiempo[i]*np.sqrt(((divi)*(c**2))-(k/m))))
            valor3Arriba = ((1/(m*x0))*((m*v0)+(c*x0))-((1/(2*m))*c))
            valor3Abajo = (np.sqrt((divi)*(c**2)-(k/m)))
            valor3 = valor3Arriba/valor3Abajo
            valor4 = (np.sinh(xTiempo[i]*np.sqrt(((divi)*(c**2))-(k/m))))
            xEsperado[i] = (valor1*(valor2+valor3*valor4))
            valor1a[i]=valor1
            valor2a[i]=valor2
            valor3a[i]=valor3
            valor4a[i]=valor4
        
        print(f'valor1 = {valor1a[2]}') 
        print(f'valor2 = {valor2a[2]}') 
        print(f'valor3 = {valor3a[2]}') 
        print(f'valor4 = {valor4a[2]}') 
        print(f'valor de la posicion 1 es {xEsperado[2]}')
        
        #print(f'valor total es {xEsperado}')
        #print(f'tiempo total es {xTiempo}')
        return xTiempo, xEsperado
         
    ValorEsperadoPosicion = [ValorMedioPosicion(x0_value, v0_value, c_value, k_value, m_value, tf_value, dt_value) for _ in range(1)]
    ...
    def ValorMedioVelocidad(x0, v0, c, k, m, tf, dt):
        ...
        divi = 1/por4
        for i in range(0, len(xTiempo)):
            valor1 = (v0*np.exp((-1/(2*m))*(xTiempo[i]*c)))
            valor2 = (np.cosh(xTiempo[i]*np.sqrt(((divi)*(c**2))-(k/m))))
            valor3Arriba = (((1/(2*m))*c)+((k/(m*v0))*x0))
            valor3Abajo = (np.sqrt((divi)*(c**2)-(k/m)))
            valor3 = valor3Arriba/valor3Abajo
            valor4 = (np.sinh(xTiempo[i]*np.sqrt(((divi)*(c**2))-(k/m))))
            xEsperado[i] = (valor1*(valor2-valor3*valor4))
    ...
    ValorEsperadoVelocidad = [ValorMedioVelocidad(x0_value, v0_value, c_value, k_value, m_value, tf_value, dt_value) for _ in range(1)]
    # Trayectorias de posición
    plt.subplot(2, 1, 1)
    for i, (xTiempo, xValue, vValue) in enumerate(trayectorias):
        plt.plot(xTiempo, xValue, color="red")
        
    for i, (xTiempos, xEsperado) in enumerate(ValorEsperadoPosicion):
        plt.plot(xTiempos, xEsperado, color='black', lw=3)
    plt.xlabel('Tiempo')
    plt.ylabel('Posición')
    ...
    for i, (xTiempos, xEsperado) in enumerate(ValorEsperadoVelocidad):
        plt.plot(xTiempos, xEsperado, color='black', lw=3)
    plt.xlabel('Tiempo')
